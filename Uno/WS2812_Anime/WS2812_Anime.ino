...
void setup() {
  ...
  // scale max luminosity to 1/8 
  for (int i = 0; i < NUM_COLORS; i++) {
    colors[i].r = colors[i].r >> 3;
    colors[i].g = colors[i].g >> 3;
    colors[i].b = colors[i].b >> 3;
  }
...
// ***************************************************
void FadeInOut2(const CRGB color, int SpeedDelay) {
  byte r, g, b;
  r = color.r;
  g = color.g;
  b = color.b;
  FadeInOut(r, g, b, SpeedDelay);
}

void FadeOut(int SpeedDelay) {
  CRGB buff[NUM_LEDS];
  float scale;
...
void setPixel2(int Pixel, CRGB color) {
  leds[Pixel] = color;
}
