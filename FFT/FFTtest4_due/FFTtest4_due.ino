 ...
 * 
 * The signal is then converted to the frequency domain with an FFT.
 * Then the magnitude of the frequency domain is computed (spectrum)
 * 
 * The 5 highest peaks are extracted from the 66Hz-900Hz range of 
 * the spectrum. These peaks are processed in order to find the 
...
#define LOW_PEAK_POWER   200    // lowest peak power considered in peak search
...
const int samplingPeriod = round(1000000.0/samplingFrequency);  // in microsecs
 ...
 * since the minimum audible frequency is approx. 20 Hz
 */
...
void loop()
{
  ...
  // totalPower = findTotalPower(vReal, nSample);
  list = findPeakList(vReal, startIndex, stopIndex, nSampleFFT, samplingFrequency);
  fundamental = findFundamentalFrequency(list);


  char s[100];
  // sprintf(s, "POWER: %6.0f,  ", totalPower); Serial.print(s);
  // sprintf(s, "NPEAK: %1d,  ", list.nPeak); Serial.print(s);
  sprintf(s, "F: %03.0f  ", fundamental.freq); Serial.print(s);
 
//  if (list.nPeak > 0) {
//    for (int i = 0; i < list.nPeak; i++) {
//      sprintf(s, "|  %3.0f Hz  %4.0f W  ", list.peak[i].freq, list.peak[i].power);
//      Serial.print(s); 
//    }
//  }
  Serial.println();
